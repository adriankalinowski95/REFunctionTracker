...
#define SPLIT_OPERATOR ','
#define GET_VALUE_FROM_ADDRESS_START "["
#define GET_VALUE_FROM_ADDRESS_END "]"
#define PLUS_OPERATOR "+"
#define MINUS_OPERATOR "-"



const char* REGISTER_MNEMONICS[] = 
...
int MnemonicsX64::getRegisterIndex(std::string reigsterMnemonic)
{
	for (int i = 0; i < sizeof(REGISTER_MNEMONICS) / sizeof(const char*); i++)
	{
		if (strcmp(reigsterMnemonic.c_str(), REGISTER_MNEMONICS[i]) == 0)
		{
			return i;
		}
	}
...
int MnemonicsX64::getRegisterByIndex(int registerIndex, std::string* registerMnemonic)
{
	if (registerIndex > sizeof(REGISTER_MNEMONICS))
	{
		return -1;
	}
	else
	{
		registerMnemonic->assign(REGISTER_MNEMONICS[registerIndex]);
		return 1;
	}
}

int MnemonicsX64::getInstructionIndex(std::string instructionMnemonic)
{


	int instructionMnemonicsSize = sizeof(INSTRUCTION_MNEMONICS);
	int startIndex, offset;

	char temp[255];

	for (int i = 0; i < instructionMnemonicsSize; i++)
	{
		if (((int)INSTRUCTION_MNEMONICS[i] < 65 || (int)INSTRUCTION_MNEMONICS[i] >122) && (int)INSTRUCTION_MNEMONICS[i] > 0)
		{
			startIndex = i;
			offset = INSTRUCTION_MNEMONICS[i];
			strncpy_s(temp, INSTRUCTION_MNEMONICS + startIndex + 1, offset + 1);
			temp[offset] = '\0';
			if (strcmp(instructionMnemonic.c_str(), temp) == 0)
			{
				return i;
			}
		}
...
int MnemonicsX64::getInstructionByIndex(int instructionIndex,std::string* instructionMnemonic)
{
	...
	for (int i = 0; i < instructionMnemonicsSize; i++)
	{
		if (((int)INSTRUCTION_MNEMONICS[i] < 65 || (int)INSTRUCTION_MNEMONICS[i] >122) && (int)INSTRUCTION_MNEMONICS[i] > 0)
		{
			if (i == instructionIndex)
			{
				...
				strncpy_s(temp, INSTRUCTION_MNEMONICS + startIndex + 1, offset + 1);
				temp[offset] = '\0';
				instructionMnemonic->assign(temp);
				return 1;
...
MnemonicsX64::InstructionAnalyse* MnemonicsX64::getInstructionAnalyze(AssemblerInstruction* instruction)
{
	
	MnemonicsX64::InstructionAnalyse* instructionAnalyse = new MnemonicsX64::InstructionAnalyse;

	std::string operands(instruction->getOperands());
	std::vector<std::string> splitedMnemonic = split(operands, SPLIT_OPERATOR);
	...
	*/
	...
	/*
	while (token != NULL) {
		splitedMnemonic.push_back(token);
		token = std::strtok(NULL, SPLIT_OPERATOR);
	}
	*/
	...
	if (splitedMnemonic.size() == 0)
	{
		return instructionAnalyse;
	}

	if (splitedMnemonic.size() == 2)
	{
		std::transform(splitedMnemonic.at(0).begin(), splitedMnemonic.at(0).end(), splitedMnemonic.at(0).begin(), std::toupper);
		std::transform(splitedMnemonic.at(1).begin(), splitedMnemonic.at(1).end(), splitedMnemonic.at(1).begin(), std::toupper);

		InstructionArgument* firstInstructionArgument = new InstructionArgument;
		InstructionArgument* secondInstructionArgument = new InstructionArgument;

		int firstPart = getInstructionArgument(splitedMnemonic.at(0), firstInstructionArgument);
		int secondPart = getInstructionArgument(splitedMnemonic.at(1), secondInstructionArgument);

		if (firstPart == 1)
		{
			instructionAnalyse->arguments.push_back(firstInstructionArgument);
		}
		if (secondPart == 1)
		{
			instructionAnalyse->arguments.push_back(secondInstructionArgument);
		}
		return instructionAnalyse;
	}
	if (splitedMnemonic.size() == 1)
	{
		std::transform(splitedMnemonic.at(0).begin(), splitedMnemonic.at(0).end(), splitedMnemonic.at(0).begin(), ::toupper);
		InstructionArgument* firstInstructionArgument = new InstructionArgument;
...
int MnemonicsX64::getInstructionArgument(std::string instructionPart, InstructionArgument* instructionArgument)
{
	
	int startBracketIndex0 = instructionPart.find(GET_VALUE_FROM_ADDRESS_START);
	int endBracketIndex0 = instructionPart.find(GET_VALUE_FROM_ADDRESS_END);


	if (startBracketIndex0 != std::string::npos && endBracketIndex0 != std::string::npos)
	{
		int plusIndex = instructionPart.find(PLUS_OPERATOR);
		int minusIndex = instructionPart.find(MINUS_OPERATOR);
		if (plusIndex != std::string::npos)
		{
			std::string registerMnemonic = instructionPart.substr(startBracketIndex0 + 1, plusIndex - startBracketIndex0 - 1);
			std::string offsetValue = instructionPart.substr(plusIndex + 1, endBracketIndex0 - plusIndex - 1);

			int registerIndex = this->getRegisterIndex(registerMnemonic);
			...
			//instructionArgument = new InstructionArgument;
			sscanf_s(offsetValue.c_str(), "0X%I64X", &instructionArgument->offsetAddress);
			instructionArgument->registerIndex = registerIndex;
			strcpy_s((char*)instructionArgument->registerMnemonic.c_str(), strlen(registerMnemonic.c_str()),registerMnemonic.c_str());
			instructionArgument->type = REGISTER_VALUE;
			instructionArgument->value = 0;
			return 1;
		}
		else if (minusIndex != std::string::npos)
		{
			std::string registerMnemonic = instructionPart.substr(startBracketIndex0 + 1, minusIndex - startBracketIndex0 - 1);
			std::string offsetValue = instructionPart.substr(plusIndex + 1, endBracketIndex0 - minusIndex - 2);
			...
			//instructionArgument = new InstructionArgument;
			sscanf_s(offsetValue.c_str(), "0X%I64X", &instructionArgument->offsetAddress);
			instructionArgument->offsetAddress = -instructionArgument->offsetAddress;
			instructionArgument->registerIndex = registerIndex;
			instructionArgument->registerMnemonic = registerMnemonic;
			instructionArgument->type = REGISTER_VALUE;
			instructionArgument->value = 0;
			return 1;
		}
	...
	else
	{
		...
		instructionPart.erase(std::remove(instructionPart.begin(), instructionPart.end(), ' '), instructionPart.end());
		int registerIndex = this->getRegisterIndex(instructionPart);
		if (registerIndex != -1)
		{
			//instructionArgument = new InstructionArgument;
			instructionArgument->registerIndex = registerIndex;
			instructionArgument->registerMnemonic = instructionPart;
			instructionArgument->type = REGISTER_ADDRESS;
			instructionArgument->offsetAddress = 0;
			instructionArgument->value = 0;
			return 1;
		}
		else
		{
			//instructionArgument = new InstructionArgument;
			instructionArgument->type = VALUE;
			sscanf_s(instructionPart.c_str(), "0X%I64X", &instructionArgument->value);
			instructionArgument->registerMnemonic = "";
			instructionArgument->registerIndex = -1;
			instructionArgument->offsetAddress = 0;
