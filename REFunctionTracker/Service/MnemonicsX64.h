...
{
public:
	static MnemonicsX64& getInstance()
	{
		static MnemonicsX64 mnemonicsX64;
		return mnemonicsX64;
	}
...
public: 
	...
	typedef enum
	{
		REGISTER_ADDRESS, //rejestr
		REGISTER_VALUE, //odwolanie do adresu przez który wskazuje rejestr
		VALUE			//wartoœæ
	};
...
public:
	MnemonicsX64();
	~MnemonicsX64();
	int getRegisterIndex(std::string registerMnemonic);
	int getRegisterByIndex(int registerIndex, std::string* registerMnemonic);
	int getInstructionIndex(std::string instructionMnemonic);
	int getInstructionByIndex(int instructionIndex,std::string* instructionMnemonic);
	//InstructionAnalyse* getInstructionAnalyze(_DecodedInst *instruction);
	InstructionAnalyse* getInstructionAnalyze(AssemblerInstruction* instruction);
	int getInstructionArgument(std::string instructionPart,InstructionArgument* instructionArgument);
};